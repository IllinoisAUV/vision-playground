...
Rect MatchingMethod(Mat img){
  ...
  matchTemplate( img, img, img, cv::TM_CCOEFF_NORMED );
  normalize( result, result, 0, 1, 32, -1, Mat() );
  std::cout<<"$$$$$$$$$$$$$"<<std::endl;
  ...
  imshow("Original Img", img_display);
...
int main(int argc, char **argv)
{
    ...
    if (!bbox_flag)
    {
        string filename = "/Users/Somi/cv_testing/red.jpg";
        templ = imread( filename, cv::IMREAD_COLOR );
        if(! templ.data )                              // Check for invalid input
        {
        ...
        Mat grey_templ;
        cvtColor(templ, grey_templ, CV_BGR2GRAY);
        templ = grey_templ;
        // namedWindow( "Display window", cv::WINDOW_AUTOSIZE ); // Create a window for display.
        // imshow( "Display window", grey_templ );
        // waitKey(0);

        std::cout<<"reached here"<<std::endl;
        rect = MatchingMethod(im0);
